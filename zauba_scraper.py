...
def crawl(produrl, collection):
    """Scraping the website, creating and connecting to the MOngobaseDB Database to insert the scraped data"""
    # Specifying headers
    ...
    # Determining number of pages to crawl
    try:
        pagecount = prodtree.xpath('//span[@style="float:right"]/text()')[0].strip()
        pagecount = str(pagecount).split()[-1].replace(",", "")
    except IndexError:
        # To handle the IndexError (specific exception type)
        pagecount = 0
    ...
    for page in range(int(pagecount)):
        if page == 0:
            ...
            pageurl = "https://www.zaubacorp.com/company-list/p-" + str(page + 1) + "-company.html"
        ...
        # Looping through all the companies in the page
